...
#
# Usage:
#   sh docker/bootstrap-run-fixture.sh seed
#   sh docker/bootstrap-run-fixture.sh inspect
#
...
ensure_admin_token() {
  if [ ! -f "$HOME/.botfam/token-botfam-admin-test" ]; then
    sh "$(dirname "$0")/bootstrap-test-forgejo.sh"
  fi
...
ensure_repo() {
  local token=$1
  local resp
  if ! resp="$(api_call GET "/repos/$OWNER/$REPO" "$token")"; then
    return 1
  fi

  if [ "$LAST_HTTP_CODE" != "200" ]; then
    payload='{"name":"'"$REPO"'","private":false,"auto_init":true,"default_branch":"main"}'
    resp="$(api_call POST "/orgs/$OWNER/repos" "$token" "$payload")"
...
ensure_branch() {
  ...
  ref_resp="$(api_call GET "/repos/$OWNER/$REPO/branches/$default_branch" "$token")"
  if [ "$LAST_HTTP_CODE" != "200" ]; then
    echo "bootstrap-run-fixture: unable to read base branch $default_branch for $OWNER/$REPO (HTTP ${LAST_HTTP_CODE})" >&2
    return 1
  fi
  base_sha="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("commit",{}).get("sha",""))' <<<"$ref_resp")"
  if [ -z "$base_sha" ]; then
  ...
  api_call POST "/repos/$OWNER/$REPO/git/refs" "$token" "$body" >/dev/null
  if [ "$LAST_HTTP_CODE" != "201" ]; then
...
ensure_issue() {
  local token=$1
  local issues
  if ! issues="$(api_call GET "/repos/$OWNER/$REPO/issues?state=open" "$token")"; then
    return 1
  fi
  issue_number="$(printf '%s' "$issues" | python3 -c 'import json, os, sys; title=os.environ["ISSUE_TITLE"]; issues=json.loads(sys.stdin.read() or "[]"); print(next((str(issue.get("number", "")) for issue in issues if issue.get("title")==title), end=""))')"
  ...
  issue="$(api_call POST "/repos/$OWNER/$REPO/issues" "$token" "$payload")"
  if [ "$LAST_HTTP_CODE" != "201" ]; then
...
snapshot_state() {
  ...
  printf '%s\n' "$issues" | python3 - <<'PY' > "$outdir/issue_comments_index.txt"
import json,sys
