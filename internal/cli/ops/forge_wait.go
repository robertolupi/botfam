...
Requires the token to carry the notification scope (forge-login.sh requests it).`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runForgeWait(cmd.Context(), cmd.OutOrStdout(), once, markRead,
...
func runForgeWait(ctx context.Context, out io.Writer, once, markRead bool, interval, timeout time.Duration) error {
	var actor string
	if info, err := (famconfig.GitResolver{}).ResolveIdentity("."); err == nil {
		actor = info.Actor
	}
	client, err := forge.NewClientForWorkDir(".", actor)
	if err != nil {
	...
	start := time.Now()
	for {
		ns, err := client.ListUnreadNotifications(ctx)
		if err != nil {
		...
		if len(ns) > 0 {
			...
			for _, n := range ns {
				...
				if sc, err := client.GetSubject(ctx, n.Subject.URL); err == nil {
					if sc.State != "" {
			...
			if markRead {
				for _, n := range ns {
					_ = client.MarkNotificationRead(ctx, n.ID)
				}
			}
			return nil
		}
