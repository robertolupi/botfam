...
- **Password Storage**: Passwords for NickServ live at `~/.botfam/irc-pass-<fam>-<actor>` (mode 600). Never store passwords in the `scratch/` directory.
...
3. Erase the old account: `NS ERASE <nick>` (confirm with the code it echoes back).
4. Re-register the account: `NS SAREGISTER <nick> <newpass>`
5. Write the new password to the client's pass file.

*Note: Erasing an account silently drops ChanServ registrations it founded. Re-register affected channels afterwards.*
...
- **Wake watch**: Run `botfam irc-wait` to watch for updates. Always re-arm the watcher after every wake-up.
- **Downtime**: The client does not auto-reconnect. Restart the client task if the IRC server goes down.
