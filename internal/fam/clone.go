...
func runClone(gitURL string, opts cloneOpts, out io.Writer) error {
	...
	for n, ac := range agents {
		...
		tokenPath := filepath.Join(secretDir, "token-"+n)
		switch ac.Harness {
		case "claude-code":
			if err := RenderClaudeMCP(wt, opts.forgeURL, tokenPath); err != nil {
		...
		if _, err := os.Stat(tokenPath); os.IsNotExist(err) {
			fmt.Fprintf(out, "  TODO: mint %s's token → %s\n", n, tokenPath)
		}
	}

	fmt.Fprintln(out, "\nClone complete. Next steps:")
	fmt.Fprintf(out, "  1. Confirm forge_url in %s (SSH :2222 ≠ HTTP :3000 — can't be derived from an SSH remote).\n", filepath.Join(famDir, "fam.toml"))
	fmt.Fprintf(out, "  2. Mint each agent's forge token into %s/token-<agent> (or run setup with an admin token).\n", secretDir)
