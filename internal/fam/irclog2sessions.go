...
func sessionDirname(channel string, session []ircEvent, taken map[string]bool) string {
	start := session[0].ts
	chanSuffix := ""
	if channel != "#botfam" {
...
func renderSession(channel string, session []ircEvent) string {
	start, end := session[0].ts, session[len(session)-1].ts
...
func IrcLog2SessionsCmd(args []string, out io.Writer) error {
	...
	if len(logs) == 0 {
		return errors.New("usage: botfam irclog2sessions CHATLOG [CHATLOG...] [--out <dir>] [--gap-minutes <n>] [--channel <chan>]... [--include-open]")
	}
	...
	for _, channel := range chans {
		...
		for _, session := range sessionize(chanEvents, gap) {
			...
			dir := filepath.Join(outDir, sessionDirname(channel, session, taken))
			if err := os.MkdirAll(dir, 0o755); err != nil {
				return err
			}
			path := filepath.Join(dir, "session.md")
			if err := os.WriteFile(path, []byte(renderSession(channel, session)), 0o644); err != nil {
				return err
			}
			written++
